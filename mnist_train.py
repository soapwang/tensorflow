...
def train(mnist):
	...
	cross_entropy = tf.reduce_mean(
		tf.nn.sparse_softmax_cross_entropy_with_logits(y, tf.argmax(y_, 1)))
	
	loss = cross_entropy + tf.add._n(tf.get_collection('losses'))
	...
	with tf.Session() as sess:
		...
		for i in range(TRAINING_STEPS):
			...
			if i % 1000 ==0:
				print("After %d training steps, loss on training batch is ." % (i, loss_value))
